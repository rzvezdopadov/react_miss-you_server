...
export async function getIdByEmailFromDB(email: string) {
    try {
        const answerDB = await poolDB.query('SELECT * FROM users WHERE email = $1', [email]);
...
export async function getPasswordByIdFromDB(id: number) {
    try {
        const answerDB = await poolDB.query('SELECT password FROM users WHERE id = $1', [id]);
...
export async function setJWTToDB(id: number, jwt: string) { // Set JWT in DB
    try {
        const answerDB = await poolDB.query("UPDATE users SET jwt = $1 WHERE id = $2", [jwt, id]);
...
export async function setTimecodeToDB(id: number) { // Set Time code in DB
    ...
    try {
        const answerDB = await poolDB.query("UPDATE users SET timecode = $2 WHERE id = $1", [id, timecode]);
...
export async function getJWTFromDB(id: number) { // Get JWT in DB
    try {
        const answerDB = await poolDB.query('SELECT jwt FROM users WHERE id = $1', [id]);
