import { IDialogBase, IDialogOutput, IMessage, IProfile, IProfileForDialog } from "../../interfaces/iprofiles";
import { getJWTFromDB } from "../query/auth";
import { getDialogByIdFromDB, getDialogsByIdFromDB, setDialogByIdToDB } from "../query/dialogs";
...
export async function querySetMessage(req, res) { 
    try {
        ...
        token.then((token) => {
            ...
            dialog.then((ourDialog: IDialogBase) => {
                if (!message) return res.status(400).json({ message: "Сообщение не может быть пустым!"})
                ...
                dialogSet.then(() => {
                    ...
                    dialog.then((ourDialog: IDialogBase) => {
                        ...
                        profileUser.then((profileUserData: IProfile) => {
                            const newDialog: IDialogOutput = {
                                timecode: ourDialog.timecode,
                                idUser: id,
                                name: profileUserData.name,
                                age: profileUserData.age,
                                photomain: profileUserData.photomain,
                                photolink: profileUserData.photolink,
                                messages: ourDialog.messages,
                            }
    ...
    try {
        ...
        token.then((token) => {
            ...
            dialog.then((ourDialog: IDialogBase) => {
                ...
                profileUser.then((profile: IProfile) => {
                    const newDialog: IDialogOutput = {
                        timecode: ourDialog && ourDialog.timecode || 0,
                        idUser: id,
                        name: profile.name,
                        age: profile.age,
                        photomain: profile.photomain,
                        photolink: profile.photolink,
                        messages: ourDialog && ourDialog.messages  || [],
                    }
...
export async function queryGetDialogs(req, res) { 
    try {
        ...
        token.then((token) => {
            ...
            dialogs.then((dialogs: Array<IDialogBase>) => {
                if (!dialogs.length) return res.status(200).json([]); 

                const idUsers = dialogs.map((dialog) => dialog.id1 === decode.userId ? dialog.id2 : dialog.id1);

                const users = getProfilesForDialogs(idUsers);

                users.then((users: Array<IProfileForDialog>)=>{
                    dialogs.sort((a, b) => {
                        const id1 = a.id1 === decode.userId ? a.id2 : a.id1;
                        const id2 = b.id1 === decode.userId ? b.id2 : b.id1;
                    ...
                    const newDialogs: Array<IDialogOutput> = [];
